...
Everything flows through `bh config` / `bh label` / `bh sync` and repo-provisioning tooling
...
- **No hand-edited config.** Config changes go through `bh config` / `bh label` / `bh sync`.
