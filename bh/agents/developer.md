...
1. `bh work brief <id>` — read the requirements and the printed validation command.
2. `bh work claim <id> --as <dev>` — your ack; it provisions a worktree with identity +
...
4. Self-refine: `bh work show <id>` to see the noise, then `bh work refine <id> --autosquash`
...
6. `bh work submit <id>` — hand off to async review. **Submit is not "done"**: the durable
   artifact is the `wt/bead/<id>` branch, not the worktree directory.

If review returns changes-requested, you'll be relaunched to run `bh work resume <id> --as
...
- **One ephemeral bead branch only.** Stay inside your `wt/bead/<id>` worktree. **Never** push
  `main`, open a PR, run `bh work merge`, or touch another bead — those are the merger's /
  dispatcher's job.
